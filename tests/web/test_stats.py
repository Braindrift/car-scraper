"""Tests for the stats summary view and listing detail view (CAR-8).

Covers:
- `GET /stats` rendering the empty state with no active/priced listings, and
  a populated avg-price-per-model table once rows exist.
...
def _seed_listing(
    session: Session, *, price: int | None = 189_000, active: bool = True
) -> CarListing:
    dealer = Dealer(
